...
file_path = r"26. 로또번호 시각화\lotto.xlsx"
...
df_from_excel["1등당첨금액"] = df_from_excel["1등당첨금액"].str.replace(
    pat=r"[ㄱ-ㅣ가-힣,]+", repl=r"", regex=True
)
df_from_excel["2등당첨금액"] = df_from_excel["2등당첨금액"].str.replace(
    pat=r"[ㄱ-ㅣ가-힣,]+", repl=r"", regex=True
)
df_from_excel["3등당첨금액"] = df_from_excel["3등당첨금액"].str.replace(
    pat=r"[ㄱ-ㅣ가-힣,]+", repl=r"", regex=True
)
df_from_excel["4등당첨금액"] = df_from_excel["4등당첨금액"].str.replace(
    pat=r"[ㄱ-ㅣ가-힣,]+", repl=r"", regex=True
)
df_from_excel["5등당첨금액"] = df_from_excel["5등당첨금액"].str.replace(
    pat=r"[ㄱ-ㅣ가-힣,]+", repl=r"", regex=True
)
